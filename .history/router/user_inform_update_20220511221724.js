...
router.post("/",function(req,res){
    ...
    if(PasswordChange){
        db.updateUser( PasswordChange ,originPassword, function (err,data) { 
        if(!err){
    ...
    console.log("111" + originAddress); 
    // 这是修改后的收货地址
    var AddressChange = req.body.phoneChange
    console.log("这是前端修改后的收货地址："+phoneChange);
    if(phoneChange){
        db.updateUser( phoneChange ,originPhone, function (err,data) { 
        if(!err){
            res.json({code:-1,message:"收货地址修改示波器，请重试。"})
        }else{
