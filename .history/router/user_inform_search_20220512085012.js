    ...
    db.searchUser({ phone: phone }, function (result) {
        if(result.length > 0){
            if(result[0].phone == phone){
                ...
                res.json({code:-1,message:"用户手机号查询失败，请重试1。"})
            }
        }else{
            res.json({code:-1,message:"用户手机号查询失败，请重试2。"})
        }
    });
})
// // 1.查询手机号
...
//     console.log("这是前端发送来的手机号："+password);
//     db.searchUserPassword({ password: password }, function (result) {
//         if(result.length > 0){
//             if(result[0].password == password){
//                 res.json({code : 1,message1 : result[0].password})
//             }else{
//                 res.json({code:-1,message:"用户密码查询失败，请重试1。"})
//             }
//         }else{
//             res.json({code:-1,message:"用户密码查询失败，请重试2。"})
//         }
//     });
// })
// // 1.查询手机号
